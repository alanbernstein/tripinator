...
def get_details(fname, exif):
    details = {}
    # these sections should be per-device
    # extension is close enough for now
    ...
    if fname.endswith('JPG'):
        ...
        if not 'CreateDate' in exif:
            details['error'] = 'no CreateDate'
            return details
        details['ts_str'] = exif['CreateDate']
        details['ts'] = datetime.datetime.strptime(details['ts_str'], '%Y:%m:%d %H:%M:%S.%f')
        ...
        return {'error': 'unknown format jpeg'}
    elif fname.endswith('mov'):
        # iphone video
        # these files have no GPS data
        return {'error': 'unknown format mov'}
    else:
        # these might be video thumbnails
        return {'error': 'unknown format %s' % fname}
